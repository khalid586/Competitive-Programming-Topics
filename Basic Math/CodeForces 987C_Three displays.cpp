...
{     
     ...
     repj(i,1,n)
     {
          ll pre=INT_MAX,post=INT_MAX;
          for(int j=i-1;j>=0;j--) if(size[j]<size[i]) pre = min(pre,cost[j]);
          for(int j=i+1;j<n;j++)  if(size[j]>size[i]) post = min(post,cost[j]);
          
          ll tot = pre+post+cost[i];
          ans = min(ans,tot);              
     }

     if(ans>=INT_MAX) out(-1)
     else             out(ans)
}
